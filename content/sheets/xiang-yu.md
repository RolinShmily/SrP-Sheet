---
title: "像鱼"
slug: "xiang-yu"
type: "full-score"
source: "用户提供谱例"
summary: "《像鱼》的吉他谱 PDF 与 Bilibili 多 P 演示视频整理页，对应 p=7；该 PDF 与同组曲目共用。"
instrument: "acoustic-guitar"
tuning: "E Standard"
capo: null
featured: false
publishedAt: "2026-06-03"
updatedAt: "2026-06-03"
pdf: "/assets/sheets/pdf/xiang-yu-zui-tian-qing-ge-duan-xian-mood.pdf"
bilibili:
  bvid: "BV1Pt4y1v7Bb"
  page: 7
  start: 0
  title: "像鱼"
rights: "用户提供的学习用途谱例与公开 Bilibili 演示链接；如涉及权利问题请联系移除。"
---

## 练习说明

本页整理《像鱼》的谱例 PDF 和 Bilibili 演示视频。视频来自同一个多 P 投稿，本曲目对应第 7 P。

## 内容索引

| 项目 | 信息 |
| --- | --- |
| 曲目 | 像鱼 |
| Bilibili | BV1Pt4y1v7Bb / p=7 / t=0 |
| PDF | `/assets/sheets/pdf/xiang-yu-zui-tian-qing-ge-duan-xian-mood.pdf` |

## PDF 使用

此 PDF 与《最甜情歌》《断线》《Mood》等曲目共用。打开 PDF 后按曲名定位《像鱼》谱面。

## Practice Prompt

把主旋律单独弹唱一遍，再加入低音和分解和弦。
