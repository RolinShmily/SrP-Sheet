---
title: "Love is Gone"
slug: "love-is-gone"
type: "full-score"
source: "用户提供谱例"
summary: "《Love is Gone》的吉他谱 PDF 与 Bilibili 演示视频整理页；该 PDF 与同组曲目共用。"
instrument: "acoustic-guitar"
tuning: "E Standard"
capo: null
featured: false
publishedAt: "2026-06-03"
updatedAt: "2026-06-03"
pdf: "/assets/sheets/pdf/collapsing-world-creep-free-lucky-love-is-gone-wonderful-u.pdf"
bilibili:
  bvid: "BV1KM4y1V7zJ"
  page: 1
  start: 0
  title: "Love is Gone"
rights: "用户提供的学习用途谱例与公开 Bilibili 演示链接；如涉及权利问题请联系移除。"
---

## 练习说明

本页整理《Love is Gone》的谱例 PDF 和 Bilibili 演示视频。请在共用 PDF 中按曲名定位对应谱面，再用视频确认段落进入和情绪推进。

## 内容索引

| 项目 | 信息 |
| --- | --- |
| 曲目 | Love is Gone |
| Bilibili | BV1KM4y1V7zJ / p=1 / t=0 |
| PDF | `/assets/sheets/pdf/collapsing-world-creep-free-lucky-love-is-gone-wonderful-u.pdf` |

## PDF 使用

此 PDF 与同组曲目共用。打开文件后先按曲名找到对应页，再练习主旋律、低音和过门的连接。

## Practice Prompt

慢速练习时保留完整延音，确认换把不会切断旋律线。
