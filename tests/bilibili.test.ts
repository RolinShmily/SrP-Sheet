...
describe("buildBilibiliPlayerUrl", () => {
  it("builds a player URL from a valid BVID", () => {
    expect(buildBilibiliPlayerUrl({ bvid: "BV1B7411m7LV", page: 1, start: 0 })).toBe(
      "https://player.bilibili.com/player.html?bvid=BV1B7411m7LV&p=1&t=0&autoplay=0&danmaku=0"
    );
  });

  it("includes page and start time", () => {
    const url = buildBilibiliPlayerUrl({ bvid: "BV1B7411m7LV", page: 2, start: 35 });
    expect(url).toContain("p=2");
    expect(url).toContain("t=35");
  });

  it("always disables autoplay and danmaku", () => {
    const url = buildBilibiliPlayerUrl({ bvid: "BV1B7411m7LV", page: 1, start: 0 });
    expect(url).toContain("autoplay=0");
    expect(url).toContain("danmaku=0");
  });

  it("rejects invalid BVID metadata through schema validation", () => {
    expect(() => bilibiliVideoSchema.parse({ bvid: "BVbad", page: 1, start: 0 })).toThrow();
  });
});

describe("BilibiliEmbed", () => {
  it("renders only the bare player without a description bar", () => {
    const html = renderToStaticMarkup(createElement(BilibiliEmbed, { video: { bvid: "BV1B7411m7LV", page: 1, start: 0 } }));

    expect(html).not.toContain("演示视频");
    expect(html).not.toContain("Demo title");
    expect(html).not.toContain("aria-labelledby");
    expect(html).toContain("title=\"Bilibili player BV1B7411m7LV\"");
